...
def recursiva(A,n,tol,a,b):
    while abs(a-b)>tol:
        ...
        if p2-p1!=0:
            ...
            print '[a,c]',a,',',c,'-->',(p2 -p1)
            recursiva(A,n,tol,a,c)
            print ''
        if p3-p2!=0:
            print 'segundo intervalo'
            print '[c,b]',c,',',b,'-->',(p3 -p2)
            recursiva(A,n,tol,c,b)
            print ''
    M=dema(n+1,1)
    N=dema(n+1,1)
    for i in range(n+1):
    ...
    for i  in range(1,n+1):
        ...
        if sign(N[i-1])!=sign(N[i]):
            p2+=1
    print '[',a,b,']',' contiene ',p2-p1,' autovalores'
    print 'En particular un autovalor aproximado es: ',b
    print ''
def autovBisec(A,n):
    #M=Givens(A,n,n)
